...
import { ClientRequest, IncomingMessage, request, RequestOptions } from "http"; // prettier-ignore
import { request as TlsRequest, RequestOptions as TlsRequestOptions } from "https"; // prettier-ignore
...
export class WebsocketClient extends EventEmitter {
    ...
    readonly id: string;
    readonly defaultPorts: { "ws:": string; "wss:": string };
    ...
    connect(url: string, tlsRequestOptions?: TlsRequestOptions): this {
        ...
        if (!_url.protocol)
            throw new Error(
                "Must specify a valid WebSocket URL that includes the procotol"
            );
        if (!_url.hostname)
            throw new Error(
                "Must specify a full WebSocket URL that includes hostname. Relative URLs are not supported"
            );
        ...
        if (this._secure) {
            extend(reqOptions, tlsRequestOptions ?? this._tlsOptions);
        }

        this._req = (this._secure ? TlsRequest : request)(reqOptions);
    ...
    protected _onResponse(res: IncomingMessage) {
        this._req = null;
        let headers = [];
        for (let key in res.headers) headers.push(`${key}:${res.headers[key]}`);
        this._failHandshake(
            "Server responded with non-101 status: " +
                res.statusCode +
                " " +
                res.statusMessage +
    ...
    protected _failHandshake(reason: string) {
    ...
    protected _validateHandshake() {
        ...
        if (!(headers['connection'] && headers['connection'].toLocaleLowerCase() === "upgrade")) {
             return this._failHandshake(
                 "Expected a `Connection: Upgrade` header from server"
             );
        }
        ...
        if (!headers["sec-websocket-accept"]) {
            return this._failHandshake(
                "Expecetd `Sec-WebSocket-Accept` header from server"
            );
        }

        if (headers["sec-websocket-accept"] !== hash) {
            return this._failHandshake(
                "`Sec-WebSocket-Accept` header value doesn't match with expected value: " +
                    hash
            );
        }
